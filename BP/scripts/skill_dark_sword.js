...
world.afterEvents.itemUse.subscribe((event) => {
    ...
    if (item.typeId === ITEM_ID) {
        ...
        if (isSneaking) {
            // Skill Ultimate
            player.runCommand("execute as @p run function dark_sword_2");
            player.runCommand("effect @p speed 2 1 true");
            player.runCommand("execute as @p run tag @s remove darksword");
        } else {
            // Skill Biasa
            player.runCommand("execute as @p run function dark_sword_1");
            player.runCommand("effect @p speed 2 1 true");
            player.runCommand("execute as @p run tag @s remove darksword");
