...
const ITEM_ID = "skill:battle_axe_hydro";
...
world.afterEvents.itemUse.subscribe((event) => {
    ...
    if (item.typeId === ITEM_ID) {
        ...
        // Skill Biasa
        player.runCommand("execute as @p run function hydro_1");
        player.runCommand("effect @p speed 2 1 true");
        player.runCommand("execute as @p run tag @s remove hydro");

        if (isSneaking) {
            // Skill Ultimate
            player.runCommand("execute as @p run function hydro_2");
            player.runCommand("effect @p speed 2 1 true");
            player.runCommand("execute as @p run tag @s remove hydro");
        }

        // Kirim pesan awal (trigger)
        const activationText = isSneaking ? "⌛[Ultimate:Hydro]" : "⌛[Battle Axe:Hydro]";
