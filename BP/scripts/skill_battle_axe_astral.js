...
const cooldownsSneak = new Map(); // Cooldown ultimate (18 detik)

const COOLDOWN_TIME_NORMAL = 5000; // 5 detik
const COOLDOWN_TIME_SNEAK = 18000; // 18 detik
const ITEM_ID = "skill:battle_axe_astral";
...
world.afterEvents.itemUse.subscribe((event) => {
    ...
    if (item.typeId === ITEM_ID) {
        ...
        player.runCommand("execute as @p run function ender_function_1");
        player.runCommand("effect @p speed 2 1 true");
        player.runCommand("execute as @p run tag @s remove end");

        // mode ultimate
        if (isSneaking) {
            player.runCommand("execute as @p run function ender_function_2");
            player.runCommand("effect @p speed 2 1 true");
        ...
        const activationText = isSneaking ? "⌛[Ultimate:Astral]" : "⌛[Battle Axe:Astral]";
