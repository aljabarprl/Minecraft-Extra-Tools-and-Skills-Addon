...
const cooldownsSneak = new Map(); // Cooldown ultimate (32 detik)

const COOLDOWN_TIME_NORMAL = 8000; // 8 detik
const COOLDOWN_TIME_SNEAK = 32000; // 32 detik
const ITEM_ID = "skill:battle_axe_blast";
...
world.afterEvents.itemUse.subscribe((event) => {
    ...
    if (item.typeId === ITEM_ID) {
        ...
        // Skill Biasa
        player.runCommand("execute as @p run function blast_function_1");
        player.runCommand("effect @p speed 2 1 true");
        player.runCommand("execute as @p run tag @s remove blast");

        if (isSneaking) {
            // Skill Ultimate
            player.runCommand("execute as @p run function blast_function_2");
            player.runCommand("effect @p speed 2 1 true");
            player.runCommand("execute as @p run tag @s remove blast");
        }

        // Kirim pesan awal (trigger)
        const activationText = isSneaking ? "⌛[Ultimate:Blast]" : "⌛[Battle Axe:Blast]";
