...
const cooldowns = new Map(); // Cooldown normal (7 detik)
const cooldownsSneak = new Map(); // Cooldown ultimate (30 detik)

const COOLDOWN_TIME_NORMAL = 7000; // 7 detik
const COOLDOWN_TIME_SNEAK = 30000; // 30 detik
const ITEM_ID = "skill:battle_axe_limitless";
...
world.afterEvents.itemUse.subscribe((event) => {
    ...
    if (item.typeId === ITEM_ID) {
        ...
        if (isSneaking) {
            // Skill Ultimate: Teleport ke VOID
            player.runCommand("execute as @p run function dark_2");
            player.runCommand("effect @p speed 2 1 true");
            player.runCommand("execute as @p run tag @s remove dark");
        } else {
            // Skill Biasa: Teleport ke LANGIT
            player.runCommand("execute as @p run function dark_1");
            player.runCommand("effect @p speed 2 1 true");
            player.runCommand("execute as @p run tag @s remove dark");
        }

        // Kirim pesan awal (trigger)
        const activationText = isSneaking ? "⌛[Ultimate:Darkness]" : "⌛[Battle Axe:Darkness]";
