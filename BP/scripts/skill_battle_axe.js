...
const cooldownsSneak = new Map(); // Cooldown Ultimate (28 detik)

const COOLDOWN_TIME_NORMAL = 5000; // 5 detik
const COOLDOWN_TIME_SNEAK = 28000; // 28 detik
const ITEM_ID = "skill:battle_axe_iron";
...
world.afterEvents.itemUse.subscribe((event) => {
    ...
    if (item.typeId === ITEM_ID) {
        ...
        player.runCommand("execute as @p run function thunder_function_2");
        ...
        if (isSneaking) {
            player.runCommand("execute as @p run function thunder_function_1");
            player.runCommand("effect @p speed 2 1 true");
            player.runCommand("execute as @p run tag @s remove thunder");
            ...
            // Set waktu untuk mengembalikan cuaca ke clear setelah 3 detik
            system.runTimeout(() => {
        ...
        const activationText = isSneaking ? "⚡[Ultimate:Thunder]" : "⚡[Battle Axe:Thunder]";
