...
const cooldownsSneak = new Map(); // Cooldown ultimate (26 detik)

const COOLDOWN_TIME_NORMAL = 5000; // 5 detik
const COOLDOWN_TIME_SNEAK = 26000; // 26 detik
const ITEM_ID = "skill:battle_axe_fire";
...
world.afterEvents.itemUse.subscribe((event) => {
    ...
    if (item.typeId === ITEM_ID) {
        ...
        player.runCommand("execute as @p run function fire_function");
        ...
        for (const entity of world.getDimension("overworld").getEntities({ location: player.location, maxDistance: 5, excludeNames: [player.name] })) {
            entity.setOnFire(10); // Bakar selama 10 detik
        }
        
        player.runCommand("effect @p speed 2 1 true");
        player.runCommand("execute as @p run tag @s remove fire");

        if (isSneaking) {
            player.runCommand("execute as @p run function fire_function_1");

            for (const entity of world.getDimension("overworld").getEntities({ location: player.location, maxDistance: 5, excludeNames: [player.name] })) {
                entity.setOnFire(99999); // Bakar selamanya
                entity.addEffect("slowness", 60, { amplifier: 1, showParticles: true }); // Slowness 3 detik
            }
        ...
        const activationText = isSneaking ? "⌛[Ultimate:Fire]" : "⌛[Battle Axe:Fire]";
