...
const cooldownsSneak = new Map(); // Cooldown Ultimate (20 detik)

const COOLDOWN_TIME_NORMAL = 5000; // 5 detik
const COOLDOWN_TIME_SNEAK = 20000; // 20 detik
const ITEM_ID = "skill:battle_hamma";
...
world.afterEvents.itemUse.subscribe((event) => {
    ...
    if (item.typeId === ITEM_ID) {
        ...
        player.runCommand("execute as @p run function battle_hamma_1");
        
        player.runCommand("effect @p speed 2 1 true");
        player.runCommand("execute as @p run tag @s remove battlehamma");
        player.playAnimation("animation.player.attack.positions");

        if (isSneaking) {
            player.runCommand("execute as @p run function battle_hamma_2");
            player.runCommand("effect @p speed 2 1 true");
        ...
        const activationText = isSneaking ? "⌛[Ultimate:On]" : "⌛[Battle Hamma:On]";
