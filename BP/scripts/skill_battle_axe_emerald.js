...
const cooldownsSneak = new Map(); // Cooldown ultimate (35 detik)

const COOLDOWN_TIME_NORMAL = 8000; // 8 detik
const COOLDOWN_TIME_SNEAK = 35000; // 35 detik
const ITEM_ID = "skill:battle_axe_emerald";
...
world.afterEvents.itemUse.subscribe((event) => {
    ...
    if (item.typeId === ITEM_ID) {
        ...
        // Skill Biasa
        player.runCommand("execute as @p run function emerald_1");
        player.runCommand("effect @p speed 2 1 true");
        player.runCommand("execute as @p run tag @s remove emerald");

        if (isSneaking) {
            // Skill Ultimate
            player.runCommand("execute as @p run function emerald_2");
            player.runCommand("effect @p speed 2 1 true");
            player.runCommand("execute as @p run tag @s remove emerald");
        }

        // Kirim pesan awal (trigger)
        const activationText = isSneaking ? "⌛[Ultimate:Poison]" : "⌛[Battle Axe:Poison]";
